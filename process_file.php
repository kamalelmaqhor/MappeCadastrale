...
function mappe_function($x, $y, $sc) {
    ...
    if ($sc == 20000) {
        return sprintf("%02d-%02d", $qy_20000 + 1, $qx_20000 + 1);
    } elseif ($sc == 10000) {
        return sprintf("%02d-%02d-%s", $qy_20000 + 1, $qx_20000 + 1, $dic_4[$res_10000]);
    } elseif ($sc == 5000) {
        return sprintf("%02d-%02d-%s", $qy_20000 + 1, $qx_20000 + 1, $dic_16[$res_5000]);
    } elseif ($sc == 2000) {
        return sprintf("%02d-%02d-%02d", $qy_20000 + 1, $qx_20000 + 1, $res_2000);
    } elseif ($sc == 1000) {
        return sprintf("%02d-%02d-%02d-%s", $qy_20000 + 1, $qx_20000 + 1, $res_2000, $dic_4[$res_1000]);
    } elseif ($sc == 500) {
        return sprintf("%02d-%02d-%02d-%s", $qy_20000 + 1, $qx_20000 + 1, $res_2000, $dic_16[$res_500]);
    } elseif ($sc == 200) {
        return sprintf("%02d-%02d-%02d-%02d", $qy_20000 + 1, $qx_20000 + 1, $res_2000, $res_200);
    } elseif ($sc == 100) {
        return sprintf("%02d-%02d-%02d-%02d-%s", $qy_20000 + 1, $qx_20000 + 1, $res_2000, $res_200, $dic_4[$res_100]);
    }
    return "";
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $zip = new ZipArchive();
    $zipFilename = tempnam(sys_get_temp_dir(), 'resultats') . '.zip';
    ...
    foreach ($lines as $line) {
        ...
        foreach (array(20000, 10000, 5000, 2000, 1000, 500, 200, 100) as $scale) {
            $result["scale_$scale"] = mappe_function($x, $y, $scale);
        }

        $outputContent = "Nom: $name\nX: $x\nY: $y\n";
        foreach (array(20000, 10000, 5000, 2000, 1000, 500, 200, 100) as $scale) {
            $outputContent .= "Échelle $scale : " . $result["scale_$scale"] . "\n";
        }

        $zip->addFromString($name . '.txt', $outputContent);
    }

    $zip->close();

    header('Content-Type: application/zip');
    header('Content-Disposition: attachment; filename=resultats_' . $filename . '.zip');
